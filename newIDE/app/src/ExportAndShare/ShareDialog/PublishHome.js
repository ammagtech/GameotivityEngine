...
const getSectionLabel = ({ section }: {| section: ExporterSection |}) => {
  switch (section) {
    ...
    case 'telegram':
      return <Trans>Telegram Mini App</Trans>;
    default:
      return null;
  }
};
...
const getSectionIcon = ({
  ...
  switch (section) {
    ...
    case 'telegram':
      return <Share style={small ? styles.iconSmall : styles.icon} />;
    default:
      return null;
  }
};
...
const getSubSectionIcon = (
  ...
  switch (section) {
    ...
    case 'telegram':
      return <Share style={styles.icon} />;
    case 'desktop':
    case 'android':
      switch (subSection) {
        case 'online':
...
const PublishHome = ({
  ...
  return (
    <ColumnStackLayout expand noMargin>
      ...
      {!chosenSection && (
        <ColumnStackLayout noMargin>
          ...
          <SectionLine
            ...
            icon={getSubSectionIcon('cloud', 'publish')}
            description={<Trans>Publish your game</Trans>}
            onClick={() => {
              setHasSkippedSubSectionSelection(true);
              onChooseSection('cloud');
              onChooseSubSection('publish');
            }}
            highlighted
            disabled={!isOnline}
            id="publish-publish-games"
          />
          {!showOnlineWebExporterOnly && (
            <SectionLine
          ...
          {!showOnlineWebExporterOnly && (
            <SectionLine
              label={getSectionLabel({ section: 'telegram' })}
              icon={getSectionIcon({ section: 'telegram' })}
              description={
                <Trans>Export game to telegram mini app</Trans>
              }
              onClick={() => {
                setHasSkippedSubSectionSelection(true);
                onChooseSection('telegram');
                onChooseSubSection('telegram');
              }
              }
              disabled={allExportersRequireOnline && !isOnline}
              id="publish-telegram-bot"
            />
          )}
          {/* {!showOnlineWebExporterOnly && (
            <SectionLine
      ...
      {/* for telegram-bot */}
      {chosenSection === 'telegram' && !chosenSubSection && (
        <ColumnStackLayout noMargin>
          <SectionLine
            label={<Trans>Telegram Mini App</Trans>}
            icon={getSubSectionIcon('telegram', 'telegram')}
            description={<Trans>Deploy a your game to telegram Mini App</Trans>}
            onClick={() => onChooseSubSection('telegram')}
            // highlighted
            disabled={!isOnline}
            id="publish-telegram"
          />
