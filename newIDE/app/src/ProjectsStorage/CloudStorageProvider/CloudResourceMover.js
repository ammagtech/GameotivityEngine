...
import {
  ...
  uploadProjectResourceFilesByWalletAddress,
...
// This mover can be used for both public URLs and Cloud project resources.
export const moveUrlResourcesToCloudProjectWithWA = async ({
  project,
  authenticatedUser,
  walletAddress,
  oldFileMetadata,
  ...
  const getResourcesToFetchAndUpload = async (
    ...
    const allResourceNames = resourcesManager.getAllResourceNames().toJSArray();
    const resourceToFetchAndUpload: Array<ResourceToFetchAndUpload> = [];
// Need to implement token creation for private games
    const tokenForPrivateGameTemplateAuthorization = await fetchTokenForPrivateGameTemplateAuthorizationIfNeeded(
      {
        authenticatedUser,
        allResourcePaths: allResourceNames.map(resourceName => {
  ...
  // Upload the files just downloaded, for the new project.
  await getCredentialsForCloudProjectWithWA(walletAddress, newCloudProjectId);
  const uploadedProjectResourceFiles: UploadedProjectResourceFiles = await uploadProjectResourceFilesByWalletAddress(
    walletAddress,
    // newCloudProjectId,
    downloadedFilesAndResourcesToUpload.map(({ file }) => file),
    (count, total) => {
